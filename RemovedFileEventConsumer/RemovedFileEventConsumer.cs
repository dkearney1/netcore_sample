...
{
    public sealed class RemovedFileEventConsumerService : IDisposable
    {
        ...
        public RemovedFileEventConsumerService(ILogger logger, IConnectionFactory connectionFactory)
        {
            _logger = logger;
        ...
        private async Task DoWork()
        {
            ...
            try
            {
                ...
                _channel.QueueBind(queueName, "FileEvents.FileRemoved:FileEvents", "#", null);
                    ...
                    while (!cancelToken.IsCancellationRequested)
                    {
                        try
                        {
                            ...
                            if (results != null)
                            {
                                ...
                                var fileReceived = JsonConvert.DeserializeObject<FileRemoved>(s);
                                await OnFileRemoved(fileReceived);
                            }
                        }
                        catch (Exception ex)
                        {
                ...
                _channel.QueueUnbind(queueName, "FileEvents.FileRemoved:FileEvents", "#", null);
        ...
        private Task OnFileRemoved(FileRemoved fileRemoved)
        {
            var now = DateTimeOffset.Now;
            var msg = $"FileRemoved: Id={fileRemoved.Id}, Filename={Path.GetFileNameWithoutExtension(fileRemoved.FullPath)}, Delay={(now - fileRemoved.Created).TotalMilliseconds}ms";
